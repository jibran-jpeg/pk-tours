...
const Timeline = () => {
    ...
    return (
        <section id="about" className="bg-gray-50 dark:bg-[#0a0a0a] py-20 lg:py-32 px-6 lg:px-8 overflow-hidden font-sans transition-colors duration-500">
            <div className="max-w-7xl mx-auto">
                <div className="flex items-center gap-4 md:gap-8 mb-12 lg:mb-20">
                    ...
                    <h2 className="text-3xl md:text-5xl font-light text-black dark:text-white uppercase tracking-[0.2em] font-serif text-center drop-shadow-lg text-shadow-sm px-2 transition-colors">The Expedition</h2>
                    <div className="h-px bg-black/10 dark:bg-white/20 flex-1" />
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-start">
                    <div className="space-y-8">
                        <motion.p
                            ...
                            viewport={{ once: true }}
                            className="text-gray-600 dark:text-gray-400 max-w-sm text-sm leading-relaxed uppercase tracking-wider transition-colors"
                        >
                    ...
                    <div className="relative pl-6 md:pl-8 mt-12 lg:mt-0">
                        {/* Timeline Line */}
                        <div className="absolute top-0 bottom-0 left-[2.5px] w-px bg-black/10 dark:bg-white/10" />
                        ...
                        <div className="space-y-16 lg:space-y-32">
                            {timeline.map((item, idx) => (
                                <motion.div
                                    ...
                                    className="relative"
                                >
                                    <div className="absolute top-2 -left-[25px] md:-left-[33px] w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)] dark:bg-white dark:shadow-[0_0_10px_white] transition-colors" />
                                    <div className="flex flex-col md:flex-row items-start gap-4 md:gap-8">
                                        <div className="min-w-[120px]">
                                            <p className="text-emerald-600 dark:text-white/40 text-[10px] font-bold uppercase tracking-widest mb-1 transition-colors">{item.days}</p>
                                            <h3 className="text-black dark:text-white font-bold text-lg uppercase tracking-widest transition-colors">{item.title}</h3>
                                        ...
                                        <div className="flex gap-4 w-full md:w-auto overflow-x-auto pb-4 md:pb-0 hide-scrollbar snap-x snap-mandatory mt-2 md:mt-0">
                                            {item.images.map((img, i) => (
                                                <motion.div
                                                    ...
                                                    viewport={{ once: true, margin: "100px" }}
                                                    className={`min-w-[120px] w-[140px] md:w-32 h-24 rounded-lg overflow-hidden border border-black/5 dark:border-white/5 flex-shrink-0 snap-start ${i === 1 ? 'md:mt-8' : ''}`}
                                                >
